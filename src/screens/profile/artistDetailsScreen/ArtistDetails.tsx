...
const ArtistDetails = () => {
  ...
  const [updateArtistProfile, { isLoading: updatingArtistProfileLoader }] = useUpdateArtistsProfileMutation()
  const [scrollPosition, setScrollPosition] = useState(0)
  const fadeAnim = useRef(new Animated.Value(1)).current; // Animation for opacity
  const translateAnim = useRef(new Animated.Value(0)).current; // Animation for translation
  ...
  const uploadImages = (type: string) => {
    ImagePicker.openPicker({
      ...
      // console.log("obj",obj)
      type === "banner" ? [setProfileBannerImage(obj), AddBannerPic(obj)] : setGalleryImages(obj)
    })
  }
  //Image of Artist
  const artistImage = artistProfileData?.profile?.image ? Utility.getImageUrl(artistProfileData?.profile?.image) : null
  ...
  const serviceWithCategory = artistProfileData?.services?.reduce((acc, currentItem) => {
    ...
    <ScrollView style={styles.flexContainer} showsVerticalScrollIndicator={false}>
      <View style={styles.flexContainer}>
        <View style={styles.alserviceContainer}>
          <CustomText size={20} text={`${strings?.allservice} (${artistProfileData?.services?.length})`} />
          <TouchableOpacity
            ...
            >
              {item?.category && (
                <View style={styles.categoryContainer}>
                  <CustomText color={Colors.white} text={item?.category} style={{ textAlign: "left" }} />
                </View>
              )}
            </ImageBackground>
          )
  ...
  const fourthRoute = () => (
    <ScrollView showsVerticalScrollIndicator={false}>
      <View style={styles.buttonContainer}>
        ...
        <TouchableOpacity
          onPress={() => {
            setAboutUs(artistProfileData?.aboutus?.description || "")
            descriptionSheetRef?.current?.open()
          }}
          activeOpacity={strings.buttonopacity}
          style={styles.flex}
        >
          <CustomText color={Colors.primary} size={14} text={strings.edit_About} />
      ...
      {artistProfileData?.workinghours?.map((item, index) => {
        return (
          <View key={index} style={styles.sheetContainer4}>
            <CustomText color={Colors.lightGrey} size={16} text={`${item?.starttime}-${item?.endtime}`} />
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={[styles.topContainer, scrollPosition >= 100 && { marginTop: 0 }]}>
      <Animated.View
        style={[
          styles.animatedContainer,
          {
      ...
      {scrollPosition < 100 && (
        <View style={styles.iconMainContainer}>
          <View style={styles.artistContainer}>
            <CustomText color={Colors.lightGrey} size={14} text={"@artistuser"} />
            <Image style={styles.instaImage} source={Images.insta} />
          </View>
          <View style={styles.avatarContainer}>
            <TouchableOpacity style={styles.editImage} onPress={() => navigation.navigate("YourProfile")}>
              <Image source={Images.edit} />
            </TouchableOpacity>
            <Image style={styles.profilePicStyle} source={{ uri: artistImage }} defaultSource={Images.profilepic} />
          </View>
          <View style={styles.artistContainer}>
            <CustomText color={Colors.lightGrey} size={14} text={"@artistuser"} />
        ...
        >
          <View style={styles.profileSection}>
            ...
            <View style={styles.jennyContainer}>
              ...
              <View style={styles.artistContainer}>
                <CustomText color={Colors.lightGrey} size={16} text={strings.jobdone} />
                <Text style={styles.jobCancleLine}> | </Text>
                <CustomText color={Colors.lightGrey} size={16} text={strings.jobcancel} />
              </View>
              <View style={styles.artistContainer}>
                <Image style={styles.clockImage} source={Images.clock} />
                <CustomText color={Colors.lightGrey} size={15} text={strings.min15} />
                <View style={styles.dot} />
                <CustomText color={Colors.lightGrey} size={15} text={strings.km15} />
                <View style={styles.dot} />
                <CustomText color={Colors.lightGrey} size={15} text={strings.pm11} />
                <CustomText color={Colors.lightGrey} size={15} text={strings.am11} />
                <Text style={styles.monsunText}> | </Text>
                <CustomText color={Colors.lightGrey} size={15} text={strings.monsun} />
              </View>
        ...
        >
          <ScrollView contentContainerStyle={[styles.contentContainer, { paddingBottom: 50 }]}>
            ...
            {schedule.map((item, index) => (
              <Pressable key={index} style={styles.sheetContainer} onPress={() => showDatePicker("startTime", index)}>
                <CustomText color={Colors.lightGrey} size={16} text={item.date} />
                <View style={styles.flex}>
                  <CustomText size={14} fontWeight="400" text={`${item.startTime} - ${item.endTime}`} />
        ...
        >
          <View style={styles.contentContainer}>
            <CustomText style={styles.alignText} size={22} text={strings?.addphotos} />
            <View style={styles.divider} />
            <CustomText size={18} text={strings?.uploadworkphotos} style={{ textAlign: "left" }} />
            <CustomText size={15} color={Colors.lightGrey} text={strings?.lorem_ipsum} style={{ textAlign: "left" }} />
            <DottedBorder
        ...
        >
          <View style={styles.contentContainer}>
            <CustomText style={styles.alignText} size={22} text={strings?.adddesc} />
            <View style={[styles.divider, { marginBottom: 17 }]} />
            <CustomText text={strings.description} style={{ textAlign: "left" }} />
            <TextInput
              // numberOfLines={6}
        ...
        >
          <ScrollView contentContainerStyle={[styles.contentContainer, { paddingBottom: 50 }]}>
            <CustomText style={styles.alignText} size={22} text={strings?.contactndadress} />
            <View style={[styles.divider, { marginBottom: 17 }]} />
        ...
        >
          <ScrollView contentContainerStyle={[styles.contentContainer, { paddingBottom: 50 }]}>
            ...
            <CustomInput
              ...
              onChangeText={(text) => setProfileDetail((prev) => ({ ...prev, phone: text }))}
            />
            <CustomInput
              style={styles.inputStyle}
              ...
              label={strings.email}
              editable={false}
              onChangeText={(text) => setProfileDetail((prev) => ({ ...prev, email: text }))}
            />
            {/* <CustomInput
              style={{marginTop: 10}}
              label={strings.email}
            ...
            <CustomInput
              ...
              onChangeText={(text) => setProfileDetail((prev) => ({ ...prev, address: text }))}
            />
            {/* <CustomInput
              style={{marginTop: 10}}
              label={strings.address}
...
const styles = StyleSheet.create({
  ...
  profileSection: {
    // Add styles for the profile section here if needed
  },
  inputStyle: {
    marginTop: 10,
  },
